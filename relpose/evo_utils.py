...
def load_7scenes_traj(gt_dir: str) -> tuple:
    pose_files = sorted(Path(gt_dir).glob("*.pose.txt"))
    if not pose_files:
        raise FileNotFoundError(f"No 7Scenes pose files found in {gt_dir}")
    tum_gt_poses = []
    for pose_file in pose_files:
        c2w = np.loadtxt(pose_file)
        xyz = c2w[:3, -1]
        quat_xyzw = Rotation.from_matrix(c2w[:3, :3]).as_quat()
        wxyz = np.array([quat_xyzw[-1], quat_xyzw[0], quat_xyzw[1], quat_xyzw[2]])
        tum_gt_poses.append(np.concatenate([xyz, wxyz], 0))
    tum_gt_poses = np.stack(tum_gt_poses, 0)
    timestamps_mat = np.arange(len(tum_gt_poses)).astype(float)
