...
EVAL_DEPTH_METADATA = {
    ...
    "tum_dynamic": {
        "depth_read_func": lambda filename: depth_read_tum(filename),
        "depth_evaluation_kwargs": {
            "max_depth": 70,
            "use_gpu": True
        },
    ...
    "7scenes": {
        "depth_read_func": lambda filename: depth_read_7scenes(filename),
        "depth_evaluation_kwargs": {
            "max_depth": 10,
            "use_gpu": True
        },
...
def depth_read_tum(filename: str):
    depth_png = np.asarray(Image.open(filename))
    assert np.max(depth_png) > 255
    depth = depth_png.astype(np.float64) / 5000.0
    depth[depth_png == 0] = -1.0
...
def depth_read_7scenes(filename: str):
    depth_png = np.asarray(Image.open(filename))
    assert np.max(depth_png) > 255
    depth = depth_png.astype(np.float64) / 1000.0
    depth[(depth_png == 0) | (depth_png == 65535)] = -1.0
    return depth
