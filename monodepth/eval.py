...
def main(hydra_cfg: DictConfig):
    ...
    for idx_dataset, dataset_name in enumerate(all_eval_datasets, start=1):
        ...
        for idx_seq, seq in enumerate(seq_list, start=1):
            ...
            if max_frames > 0:
                seq_gt_paths = seq_gt_paths[:max_frames]
                seq_pred_paths = seq_pred_paths[:max_frames]
            ...
            time_path = osp.join(output_root, seq, "_time.json") if dataset_info.type == "video" else osp.join(output_root, "_time.json")
            if osp.isfile(time_path):
                with open(time_path, "r") as f:
                    timing = json.load(f)
                all_fps.append(float(timing.get("fps", 0.0)))
                all_times.append(float(timing.get("time", 0.0)))
                all_frames.append(int(timing.get("frames", len(seq_gt_paths))))
        ...
        if all_times:
            ...
            average_metrics["fps"] = total_frames / total_time if total_time > 0 else 0.0
            average_metrics["total_time"] = total_time
            average_metrics["frames"] = total_frames
