...
RUN_ROOT="${RUN_ROOT:-$ROOT/new_results/1/layer4_dynamic_segmentation_crf_tum300_allframes_pca128_k03_equalframe_attention}"
GPUS=(6 7)
SEQUENCES_CSV="${SEQUENCES_CSV:-}"

mkdir -p "$RUN_ROOT/logs"
mapfile -t SEQUENCES < <(find "$TUM_ROOT" -mindepth 1 -maxdepth 1 -type d -name 'rgbd_dataset_*' -printf '%f\n' | sort)
if [[ -n "$SEQUENCES_CSV" ]]; then
...
run_worker() {
  ...
  for sequence in "${SEQUENCES[@]}"; do
    ...
    output="$RUN_ROOT/tum_dynamic/$sequence"
    printf '[%s] gpu=%s sequence=tum_dynamic/%s\n' "$(date '+%F %T')" "$gpu" "$sequence" | tee -a "$log"
    ...
    "$PYTHON_BIN" "$ROOT/scripts/visualize_layer4_token_clusters.py" \
      ...
      --max-source-frames 300 --num-frames 300 --processing-window-size 10 \
      --pca-dim 128 --clusters 3 --seed 0 --quality-sample-size 3000 --label-smoothing crf \
      --camera-attention --camera-attention-global-dynamic --overwrite >> "$log" 2>&1
    ((index += 1))
  done
}
