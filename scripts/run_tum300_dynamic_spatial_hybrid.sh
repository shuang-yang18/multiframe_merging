...
GPU="${1:?usage: run_tum300_dynamic_spatial_hybrid.sh <gpu> <all|middle|late|middle_late>}"
SCHEDULE="${2:?usage: run_tum300_dynamic_spatial_hybrid.sh <gpu> <all|middle|late|middle_late>}"
...
case "$SCHEDULE" in
  ...
  *) echo "Unknown dynamic FastVGGT schedule: $SCHEDULE" >&2; exit 2 ;;
esac
...
"$PYTHON_BIN" inference/infer.py \
  ...
  --output-dir "$RUN_ROOT/tum300_dynamic_spatial_hybrid_r${MERGE_RATIO}_${SCHEDULE}" \
  --overwrite \
  --eval \
  ...
  --token-merging-method dynamic_spatial_hybrid \
